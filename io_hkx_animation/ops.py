        ...
        if armature.animation_data and armature.animation_data.action:
            for marker in armature.animation_data.action.pose_markers:
                if marker.frame >= self.frame_interval[0] and marker.frame <= self.frame_interval[1]:
                    ...
                    text = marker.name
                    # [Skyrim Annotation Support] Try to get full text
                    if hasattr(armature.animation_data.action, "marker_long_strings"):
                        for item in armature.animation_data.action.marker_long_strings:
                            if item.frame == marker.frame:
                                text = item.full_text
                                break
                                
                    ianim.add_annotation(i, text)
